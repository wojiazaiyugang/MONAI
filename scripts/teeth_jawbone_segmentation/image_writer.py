...
"""
Modified from MONAI/data/image_writer.py, dev branch, SHA: a676e3876903e5799eb3d24245872ea4f3a5b152
"""
...
from monai.data.utils import ensure_tuple, ensure_tuple_rep, to_affine_nd
...
class ITKWriter(ImageWriter):
    ...
    def create_itk_image(self, verbose: bool = False, **kwargs):
        """
        Create an ITK object from ``self.create_backend_obj(self.obj, ...)``
        """
        filename = "dumpy_filename"
        super().write(filename, verbose=verbose)
        data_obj = self.create_backend_obj(
            self.data_obj,
            channel_dim=self.channel_dim,
            affine=self.affine,
            dtype=self.output_dtype,  # type: ignore
