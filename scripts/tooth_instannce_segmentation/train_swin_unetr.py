...
from monai.transforms import (
    ...
    Orientationd,
    RandShiftIntensityd,
    EnsureTyped, CropForegroundd, RandCropByPosNegLabeld,
    SpatialCropd, MapLabelValued
)
...
from scripts.tooth_instannce_segmentation.config_swin_unetr import scale_intensity_range, IMAGE_SIZE, work_dir, \
...
train_transforms = Compose(
    [
        ...
        EnsureTyped(keys=["image", "label"], device=device, track_meta=False, dtype=torch.float16),
        # MapLabelValued(keys="label", orig_labels=[i for i in range(1, 33)], target_labels=[1 for i in range(1, 33)]),
...
val_transforms = Compose(
    [
        ...
        Orientationd(keys=["image", "label"], axcodes="RAS"),
        scale_intensity_range,
        EnsureTyped(keys=["image", "label"], device=device, track_meta=True, dtype=torch.float16),
        # MapLabelValued(keys="label", orig_labels=[i for i in range(1, 33)], target_labels=[1 for i in range(1, 33)]),
        RandCropByPosNegLabeld(
            keys=["image", "label"],
            label_key="label",
...
def validation(epoch_iterator_val):
    ...
    with torch.no_grad():
        for step, batch in enumerate(epoch_iterator_val):
            ...
            if step == 0:
                slice_id = 0
                for slice_id in range(0, IMAGE_SIZE[2], 10):
                    label = val_labels[0][0][..., slice_id]
                    if int(label.unique().size()[0]) > 5:
                        break
                ...
                pred = torch.argmax(val_outputs, dim=1)[0][..., slice_id]
                for i in range(1, CLASS_COUNT):
                    gt_image[label == i] = colors[i]
                    pred_image[pred.cpu().numpy() == i] = colors[i]
