...
from scripts.pretrain.config_swin_unetr import IMAGE_SIZE, CLASS_COUNT, DATASET_DIR, CACHE_DIR


def main():
    logdir_path = os.path.normpath(str(Path(__file__).parent.joinpath("logs").joinpath("swin_unetr").joinpath("1")))

    datas = list(DATASET_DIR.glob("*.nii.gz"))
    ...
    check_ds = PersistentDataset(data=train_Data, transform=train_Transforms, cache_dir=str(CACHE_DIR))
